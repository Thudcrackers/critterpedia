...
class FishControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create fish" do
    assert_difference('Fish.count') do
      post fish_index_url, params: { fish: { name: @fish.name, time: @fish.time, value: @fish.value } }
    end
  ...
  test "should update fish" do
    patch fish_url(@fish), params: { fish: { name: @fish.name, time: @fish.time, value: @fish.value } }
